...
def window_1():
    window("Total population of each District","SELECT District_Code, State_UT, District, Population FROM census_2011")

# Literate male and female in each district
def window_2():
    window("Literate Male and Female in each district", "SELECT District_Code, State_UT, District, Literate_Male, Literate_Female FROM census_2011")
...
def window_3():
    window("Percentage of Workers in each district",
            "SELECT District_Code, State_UT, District, ROUND((Workers/Population)*100, 2) \
                            AS Percentage_of_Workers FROM census_2011")

# Number of households have access to LPG or PNG as a cooking fuel in each district
def window_4():
    window('Households with LPG or PNG as Cooking fuel',
            "SELECT District_Code, State_UT, District, LPG_or_PNG_Households FROM census_2011")
...
# Number of households have internet access in each district
def window_6():
    window("Households with Internet in each District", 
            "SELECT District_Code, State_UT, District, Households_with_Internet FROM census_2011")
...
def window_7():
    window("Educational attainment distribution in each district",
            "SELECT District_Code, State_UT, District, Below_Primary_Education, Primary_Education, Middle_Education, \
                ...
                Illiterate_Education, Total_Education FROM Census_2011")

# Number of households have access to various modes of transportation (bicycle, car, radio, television, etc.) in each district
def window_8():
    window("Households with access to various modes of transports in each district", 
            "SELECT District_Code, State_UT, District, Households_with_Bicycle, Households_with_Car_Jeep_Van, \
                Households_with_Radio_Transistor, Households_with_Scooter_Motorcycle_Moped FROM census_2011")
...
# Number of households have a latrine facility within the premises in each state
def window_12():
...
def window_13():
    window("Average household size in each state",
            ...
            FROM census_2011 GROUP BY State_UT")

# Number of households are owned versus rented in each state
def window_14():
...
# Number of households have access to drinking water sources near the premises in each state
def window_16():
...
# Number of households fall below the poverty line in each state based on the power parity categories
def window_19():
    window("Households below the poverty line in each state",
            "SELECT State_UT, SUM(Power_Parity_Less_than_Rs_45000) AS Households_below_poverty_line FROM census_2011 \
            GROUP BY State_UT")
...
def main():
    ...
    options = ['Total population of each District', 'Literate Male and Female in each district',
    ...
    select = st.selectbox("Please select a query to display :",options = options, index = None, placeholder = 'Choose an option')
    if select == 'Total population of each District':
        window_1()
